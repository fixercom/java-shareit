...
class UserServiceImplTest {
    ...
    @Test
    void updateUser_whenEmailIsAlreadyInUse_throwException() {
        Long userId = 5L;
        String newEmail = "der@q.ry";
        User oldUser = User.builder().id(userId).name("Ivan").email("rew@tu.com").build();
        UserDto userDto = UserDto.builder().email(newEmail).build();
        when(userRepository.findById(anyLong())).thenReturn(Optional.of(oldUser));
        when(userRepository.existsByEmail(anyString())).thenReturn(true);

        assertThatThrownBy(() -> userService.updateUser(userId, userDto))
                .isInstanceOf(EmailIsAlreadyInUseException.class)
                .hasMessage("Email address %s is already used", newEmail);
    }
